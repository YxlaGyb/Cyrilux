...
def compute_perplexity(pc_model, loader, pos_emb, gamma=0.1, T=2, max_batches=20):
    ...
    for batch_idx, (input_ids, labels) in enumerate(loader):
        ...
        if T > 0:
            z_by_layer, _, _, _ = pc_model.spatiotemporal_infer(
                z_by_layer, pos, gamma=gamma, T=T
            )

        # CE loss
        h_norm = pc_model.model.norm(z_by_layer[pc_model.num_sub_layers])
        logits = pc_model.model.lm_head(
            h_norm.to(dtype=pc_model.model.lm_head.weight.dtype)
        )
...
@torch.no_grad()
def generate_text(
    pc_model, prompt, max_new_tokens=50, gamma=0.1, temperature=0.8, top_k=20
):
    """字节级自回归文本生成。
...
def generate_text_manual(
    pc_model, prompt, pos_emb, max_new=50, gamma=0.1, T=2, temp=0.8, top_k=20
):
    """手动字节级自回归生成 (支持 PC 推理)。
...
def eval_self_supervised(pc_model, loader, pos_emb, gamma=0.1, T=2, max_batches=20):
    ...
    for batch_idx, (input_ids, labels) in enumerate(loader):
        ...
        valid = labels != -100
        acc = (
            (logits.argmax(-1)[valid] == input_ids[valid]).half().mean().item()
            if valid.any()
            else 0.0
        )
    ...
    for ℓ in range(1, pc_model.num_sub_layers + 1):
        ...
        summary["per_layer"][lk] = {
            ...
            "smoothness": sum(metrics["smoothness"][lk])
            / len(metrics["smoothness"][lk]),
...
def create_eval_loader(
    data_path: str, max_length=128, max_samples=500, batch_size=8, shuffle=False
):
    """创建评估用的 DataLoader。"""
...
def run_full_evaluation(
    ...
    for name, (model, pos) in models.items():
        ...
        # 自监督
        try:
            ss = eval_self_supervised(
                model, loader_ss, pos, gamma=gamma, T=T, max_batches=max_batches
            )
            results_ss[name] = ss
            Logger(
                f"  F_total: {ss['F_mean']:.4f}  Recon Acc: {ss['recon_acc_mean']:.4f}"
            )
            for l, m in ss["per_layer"].items():
                Logger(
                    f"{l}: sparsity={
                        m['sparsity']:.4f} smooth={
                            m['smoothness']:.4f} var={m['variance']:.4f}"
                )
        ...
        try:
            ...
            Logger(
                f"  T=0: PPL={ppl0:.2f}  T={T}: PPL={ppl2:.2f} ({
                    '改善' if ppl2 < ppl0 else '劣化'} {abs(delta):.1f}%)"
            )
        except Exception as e:
        ...
        try:
            ...
            for prompt in prompts:
                text = generate_text(
                    model, prompt, max_new_tokens=40, temperature=0.8, top_k=20
                )
    ...
    Logger("═" * 60)
    header = f"{'Model':25s} {
        'F↓':>10s} {'Recon↑':>8s} {'PPL(T=0)↓':>12s} {'PPL(T=2)↓':>12s}"
    Logger(header)
    Logger("─" * 70)
    for name in models:
