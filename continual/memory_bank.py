...
class MemoryBank:
    ...
    def __init__(self, max_per_task: int = 2000, enable_intrinsic_eviction: bool = True):
    ...
    def add_samples(
        ...
        for byte_t, label_t in samples:
            ...
            buf.append(Exemplar(
                ...
                replay_priority=max(dopamine_score, 0.1) + max(transition_surprise, 0.0) + intrinsic_value,
    ...
    def _evict(self, task_id: str):
        ...
        if self.enable_intrinsic_eviction:
            ...
            for i, ex in enumerate(buf):
                ...
                decay = math.exp(-self.max_per_task * 0.001 * (1.0 / max(ex.intrinsic_value, 0.01)) * max(age, 1))
    ...
    def sample(self, batch_size: int, strategy: str = 'dopamine') -> List[Exemplar]:
        ...
        for buf in self._store.values():
            for ex in buf:
                ...
                if strategy == 'dopamine':
                    w = max(ex.dopamine_score, 0.1) + max(ex.transition_surprise, 0.0) * 0.5
                    ...
                    w = (max(ex.intrinsic_value, 0.01) *
                         max(ex.replay_priority, 0.01))
        ...
        if not torch.isfinite(w).all():
            w = torch.where(torch.isfinite(w), w, torch.tensor(0.1, dtype=torch.float))
        w = w / w.sum()
        ...
        for task_id, data in state.items():
            ...
            for i in range(data['byte_tensor'].size(0)):
                buf.append(Exemplar(
                    ...
                    concept_id=(data.get('concept_ids') or [''])[i] if 'concept_ids' in data else '',
                    intrinsic_value=(data.get('intrinsic_values') or [0.0])[i] if 'intrinsic_values' in data else 0.0,
                ))
