...
def split_directory(
    ...
    for fpath in sorted(glob.glob(os.path.join(input_dir, pattern))):
        ...
        file_result = split_file(
            fpath, output_dir, chunk_size, convert, prefix=os.path.splitext(fname)[0], dry_run=dry_run
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output_dir", type=str, default=None, help="输出目录 (默认 input_split/)")
    parser.add_argument("-c", "--chunk_size", type=int, default=1000, help="每文件样本数 (默认 1000)")
    parser.add_argument("--no-convert", action="store_true", help="不自动转换格式")
    parser.add_argument("--dry-run", action="store_true", help="仅统计不分割")
    ...
    if os.path.isdir(args.input):
        results = split_directory(
            args.input, output_dir, args.chunk_size, not args.no_convert, dry_run=args.dry_run
        )
