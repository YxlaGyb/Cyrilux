...
class CUDAGraphTrainer:
    ...
    def __init__(self, loop, warmup_steps: int = 10):
        ...
        self.static_byte = torch.empty(B, 2, S, dtype=torch.bfloat16, device='cuda')
        self.static_labels = torch.empty(B, S, dtype=torch.long, device='cuda')
        self.static_precision = torch.ones(loop.model.num_sub_layers, device='cuda')

        # 世界模型上下文静态缓冲区 (仅在启用世界模型时使用)
        C = loop.cfg.get('world_model_context_dim', 2)
