"""
局部 Conv 版时空预测编码评估 — 5 维自监督指标。

Ponytail: 从 eval_pc_local.py 派生, PCLocalDynamicMiniMind + 无 RoPE.
"""
...
@torch.no_grad()
def evaluate(pc_model, loader, gamma=0.1, T=2, max_batches=20):
    """5 维自监督评估 (无 RoPE 参数)."""
    pc_model.eval()
    pos_emb = (None, None)  # 局部 Conv 无需位置编码
    ...
    for batch_idx, (input_ids, labels) in enumerate(loader):
        ...
        z_by_layer, errors_hist, F_hist = pc_model.spatiotemporal_infer(
            z_by_layer, pos_emb, gamma=gamma, T=T,
        )
...
def main():
    ...
    ckpt_path = os.path.join(ROOT, 'out_pc_local_hybrid', 'hybrid_final.pt')
    if os.path.exists(ckpt_path):
        ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
        pc_model.load_state_dict(ckpt['model_state'])
        Logger(f'Loaded checkpoint: {ckpt_path}')
    else:
        Logger(f'No checkpoint at {ckpt_path}, using untrained model')
    ...
    Logger('Running 5-dim self-supervised evaluation (local conv)...')
    summary = evaluate(pc_model, loader, gamma=0.1, T=2, max_batches=20)

    Logger('─' * 50)
    Logger('Local Conv Spatiotemporal PC Evaluation Results')
    Logger('─' * 50)
