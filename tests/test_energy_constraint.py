...
def test_first_window_lazy_init_no_excess():
    ...
    out = _energy_constraint(net, W, dW, post, "_active_ema_w35")
...
def test_excess_activates_only_above_ema():
    net = _StubNet(2)
    net._active_ema_w35.copy_(torch.tensor([1.0, 1.0], dtype=torch.float16))
    net._active_ema_init.add("_active_ema_w35")
...
def test_high_activity_gets_stronger_decay():
    net = _StubNet(3)
    net._active_ema_w35.copy_(torch.ones(3, dtype=torch.float16))
    net._active_ema_init.add("_active_ema_w35")
    W = torch.full((3, 2), 0.5, dtype=torch.float16)
    dW = torch.ones(3, 2, dtype=torch.float16)
    out_hi = _energy_constraint(
        net, W, dW, torch.tensor([[[3.0, 3.0, 3.0]]], dtype=torch.float16), "_active_ema_w35"
    )
    out_lo = _energy_constraint(
        net, W, dW, torch.tensor([[[1.0, 1.0, 1.0]]], dtype=torch.float16), "_active_ema_w35"
    )
